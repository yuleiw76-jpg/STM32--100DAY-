...
extern volatile u32 enc_a_edges;
extern volatile u32 enc_b_edges;
extern volatile u8 enc_a_level;
extern volatile u8 enc_b_level;
...
int main(void)
{
	...
	while(1)     
	{
		printf("DATA,pitch=%.3f,gyro=%d,pid=%.1f,speed=%.3f,pidv=%.1f,cnt=%u,turns=%d,pa0=%u,pa1=%u,edgeA=%lu,edgeB=%lu,ccr=%u/%u/%u/%u\r\n",
			Pitch,
			(int)gyro[1],
			pidp,
			speed,
			...
			circle_count1,
			(unsigned int)enc_a_level,
			(unsigned int)enc_b_level,
			(unsigned long)enc_a_edges,
			(unsigned long)enc_b_edges,
