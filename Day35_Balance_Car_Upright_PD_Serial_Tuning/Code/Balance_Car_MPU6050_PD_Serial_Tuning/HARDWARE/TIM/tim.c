...
u32 time_count=0;
u32 dmp_count=0;
volatile u8 dmp_miss_count=0;
volatile u8 attitude_valid=0;
volatile u32 control_tick_ms=0;
volatile u8 motor_control_enabled=0;

void Balance_Control_SetEnabled(u8 enabled)
{
	motor_control_enabled=enabled?1:0;
	...
	if (TIM_GetITStatus(TIM1, TIM_IT_Update) != RESET) //中断为定时器更新中断 
		{
			...
			if(dmp_count>=5)
			{
				...
				if(Read_DMP(&Pitch,&Roll,&Yaw))
				{
					...
					if((Pitch>(aim_pitch+35.0f))||(Pitch<(aim_pitch-35.0f)))
					{
						motor_control_enabled=0;
						go(0,0);
					}
					else if(motor_control_enabled)
					{
